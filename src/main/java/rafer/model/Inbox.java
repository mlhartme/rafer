...
public class Inbox {
    ...
    public Inbox(FileNode directory) {
    ...
    public void process(Console console, Account smugmugAccount, FolderData smugmugRoot, FileNode gpxTracks, FileNode rafs, FileNode smugmug) throws IOException {
        ...
        pairs = timestamps(console, directory);
        values = pairs.values();
        ...
        moveRafs(directory, pairs, rafs);
        if (smugmug != null) {
            console.info.println("smugmug upload ...");
            uploadJpegs(smugmugAccount, smugmugRoot, directory, pairs.keySet());
        }
